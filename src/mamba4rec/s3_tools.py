...
class s3_tools:
    ...
    def upload_file(self, file_name, bucket, object_name=None) -> bool:
    ...
    def safe_upload_folder(self, folder_name : str, bucket_name : str, object_name=None) -> bool:
        if object_name is None:
        ...
        for file_name in glob(folder_name):
            res = self.upload_file(file_name, bucket_name, object_name=object_name + os.path.basename(file_name))
            results.append(res)
    ...
    def download_folder(self, bucket_name : str, object_name : str, folder_name : str):
        ...
        for key in self.s3_client.list_objects(Bucket=bucket_name)['Contents']:
            if key.startswith(object_name) : 
                print(key['Key'])
                self.s3_client.download_file(Bucket=bucket_name, Key=key['Key'], folder_name +"/"+os.path.basename(key['Key'])) 
